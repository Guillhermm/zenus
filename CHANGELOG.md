...
- **Context Compactor** (`zenus_core/context/compactor.py`): `compact_session()` summarises the full intent history via LLM and replaces it with a single compacted entry. `maybe_compact()` auto-triggers when `token_count / max_tokens` exceeds `session.compact_threshold`. Exposed as `/compact` in the interactive shell. Fails gracefully (returns `""` and leaves history intact) if the LLM is unavailable.
- **Session Store** (`zenus_core/memory/session_store.py`): persists session snapshots to `~/.zenus/sessions/<id>.json` with `chmod 600`. Supports save, load (by ID prefix), list (newest first), delete, and restore-into. Auto-prunes oldest files when `session.max_sessions` is exceeded. Singleton via `get_session_store()`. `/session` shell command for save/load/list/delete. 14 unit tests in `tests/unit/test_phase16_session.py`.
- **TaskOps tool** (`zenus_core/tools/task_ops.py`): user-facing wrapper around the existing `BackgroundTaskQueue`. Actions: `create(command, priority)`, `list()`, `get(task_id)`, `stop(task_id)`, `output(task_id)`, `purge()`. Exposes background task management to the orchestrator and shell (`/tasks`).
- **ScheduleOps tool** (`zenus_core/tools/schedule_ops.py`): cron job management via `crontab -l`/`crontab -` with `# zenus-managed:<label>` sentinels. Actions: `schedule_cron(command, cron_expr, label)`, `list_cron()`, `remove_cron(label)`. Remote webhook triggers via `trigger_remote(url, method, payload)` with URL scheme validation (http/https only) and `requests.request()`.
- **WorktreeOps tool** (`zenus_core/tools/worktree_ops.py`): git worktree lifecycle management. `enter(branch)` calls `git worktree add` and `os.chdir()` into the isolated branch. `exit_worktree()` checks for new commits and cleans up `git worktree remove`. `current()` shows the active worktree path. Module-level state tracks `_active_worktree`, `_original_cwd`, `_active_branch`.
- **ToolSearch tool** (`zenus_core/tools/dev_ops.py`): runtime search of the tools registry by name or description. Searches at both tool level and action level; respects configurable result limit.
- **AskUserQuestion tool** (`zenus_core/tools/dev_ops.py`): pauses execution and prompts the user for structured input. Validates response against a comma-separated `options` list with retry; falls back to `default` on EOF/Ctrl-C.
- **SleepTool** (`zenus_core/tools/dev_ops.py`): waits for N seconds (max 300) — useful for polling and rate-limit back-off.
- **NotebookOps tool** (`zenus_core/tools/notebook_ops.py`): pure-JSON `.ipynb` manipulation without a kernel. Actions: `list_cells()`, `read_cell()`, `edit_cell()`, `add_cell()`, `delete_cell()`, `read_output()`, `clear_outputs()`. Validates `.ipynb` extension and file existence before operating.
